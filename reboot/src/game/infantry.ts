...
export type Infantry=WeaponState & {kind:InfantryKind;state:'idle'|'alert'|'pursue'|'attack'|'reload'|'search'|'fuse'|'panic';alert:number;reaction:number;memory:number;lastX:number;lastY:number;vy:number;grounded:boolean;ladder:number;moving:boolean;fuse:number;shield:number;attack:number};
