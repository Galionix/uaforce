import {type Enemy,type World} from './world.ts';
...
export type Infantry=WeaponState & {kind:InfantryKind;state:'idle'|'alert'|'pursue'|'attack'|'reload'|'search'|'fuse';alert:number;reaction:number;memory:number;lastX:number;lastY:number;vy:number;grounded:boolean;ladder:number;moving:boolean;fuse:number;shield:number;attack:number};
...
export const INFANTRY={
 rifle:{name:'Мотострілець РФ',hp:48,speed:3.4,sight:14,range:9,aim:.6,weapon:weapon(9,6,2,7,1.3,2.4),color:'#67724d'},
 assault:{name:'Штурмовик ВДВ',hp:60,speed:4.8,sight:15,range:7,aim:.4,weapon:weapon(7,9,3,6,.95,2.5),color:'#526956'},
 gunner:{name:'Кулеметник РФ',hp:95,speed:2.5,sight:16,range:11,aim:.85,weapon:weapon(11,12,4,7,1.6,3.4),color:'#777047'},
 sniper:{name:'Снайпер РФ',hp:38,speed:3,sight:19,range:15,aim:1.35,weapon:weapon(15,1,1,22,1,3.8),color:'#4a6450'},
 scout:{name:'Розвідник-зв’язківець РФ',hp:40,speed:4.3,sight:18,range:8,aim:.55,weapon:weapon(8,4,1,6,1.1,2),color:'#50616a'},
 shield:{name:'Щитоносець Росгвардії',hp:65,speed:2.8,sight:13,range:5,aim:.65,weapon:weapon(5,5,1,8,1,2.6),color:'#38464e'},
 demolition:{name:'Підривник-штурмовик РФ',hp:32,speed:5.4,sight:15,range:1.6,aim:.55,weapon:weapon(0,0,0,0,0,0),color:'#806449'},
} as const;
export function equipInfantry(e:Enemy,kind:InfantryKind){
 const spec=INFANTRY[kind];e.hp=e.maxHp=e.heavy?180:spec.hp;
 const a:Infantry={kind,state:'idle',alert:0,reaction:0,memory:0,lastX:e.x,lastY:e.y,vy:0,grounded:false,ladder:-1,moving:false,fuse:-1,shield:kind==='shield'?80:0,attack:0,ammo:0,reloading:0,burstShots:0,weaponTrigger:false,cooldown:0};resetWeapon(a,spec.weapon);a.cooldown=e.cooldown;e.infantry=a;return e;
}
...
function alert(w:World,e:Enemy,x:number,y:number){const a=e.infantry!;a.state='alert';a.alert=1.1;a.reaction=.55;a.memory=5;a.lastX=x;a.lastY=y;e.windup=0;w.emit('enemyAlert',e.x,e.y+2);}
export function stepInfantry(w:World,e:Enemy,dt:number){
 ...
 if(a.memory<=0){a.state='idle';e.windup=0;cycleWeapon(a,spec.weapon,dt,false);navigate(e.anchor,e.y,spec.speed*.45);return;}
