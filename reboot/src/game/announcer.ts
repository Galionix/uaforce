...
export class Announcer {
  ...
  async say(cue:Announcement){
    ...
    const schedule=(buffer:AudioBuffer,bus:GainNode,time:number,underVoiceAt?:number)=>{const source=this.ctx.createBufferSource();source.buffer=buffer;let amp:GainNode|null=null;if(underVoiceAt!==undefined){amp=this.ctx.createGain();amp.gain.setValueAtTime(1,time);amp.gain.setValueAtTime(1,underVoiceAt-.05);amp.gain.linearRampToValueAtTime(.22,underVoiceAt+.05);source.connect(amp);amp.connect(bus);this.envelopes.add(amp);}else source.connect(bus);this.sources.add(source);source.onended=()=>{this.sources.delete(source);source.disconnect();if(amp){amp.disconnect();this.envelopes.delete(amp);}if(!this.sources.size&&token===this.revision){this.until=0;this.active=null;this.duck(false);}};source.start(time);end=Math.max(end,time+buffer.duration);};
    if(riff){const lead=cue.riff==='hero'||cue.riff==='victory'?.75:.35;schedule(riff,this.musicBus,start,voices.some(Boolean)?start+lead:undefined);start+=lead;}
