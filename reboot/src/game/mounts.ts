...
export type Mount={id:number;kind:'tank';x:number;y:number;vy:number;grounded:boolean;jumpHeld:boolean;landing:number;dir:number;armor:number;maxArmor:number;cooldown:number;contactCooldown:number;moving:boolean;engine:number;hurt:number;recoil:number};
