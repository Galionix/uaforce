...
export const COOP_VERSION='uaforce-coop-12';
...
export class RemoteInput {
 private interactPending=false;private interactDown=false;
...
const fields=['survival','finale','routeProgress','mode','highFive','story','storyDone','cinematic','evac','enemies','bullets','followers','allies','ammoCrates','medkits','mounts','time','kills','shots','hits','destroyed','unlocked'] as const;
type MutableBox=Pick<Box,'id'|'x'|'y'|'hp'|'vx'|'vy'|'fuse'|'falling'|'collapseDelay'>;
export class SnapshotWriter {
 ...
 private box(b:Box):MutableBox{return {id:b.id,x:b.x,y:b.y,hp:Number.isFinite(b.hp)?b.hp:1e30,vx:b.vx,vy:b.vy,fuse:b.fuse,falling:b.falling??false,collapseDelay:b.collapseDelay??0};}
...
export function applySnapshot(w:World,s:Snapshot){
 ...
 for(const a of s.players){const target=w.players[a.id];Object.assign(target.body,a.body);target.heroId=a.heroId as HeroId;target.lives=a.lives;target.checkpoint=a.checkpoint;target.checkpointY=a.checkpointY;target.heldBarrel=a.heldBarrel;target.mounted=w.mounts.find(t=>t.id===a.mounted)??null;target.move=a.move;}
