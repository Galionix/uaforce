...
export function parseProgress(raw:string|null):Progress{
 try{const r=JSON.parse(raw??'null');if(!r)return freshProgress();const legacy:Record<string,string>={kozak:'shevchenko',kharakternyk:'lesya',scout:'franko'};r.hero=legacy[r.hero]??r.hero;r.unlocked=Array.isArray(r.unlocked)?r.unlocked.map((id:string)=>legacy[id]??id):[];const unlocked=HEROES.filter(h=>r.unlocked?.includes(h.id)).map(h=>h.id);if(!unlocked.includes('shevchenko'))unlocked.unshift('shevchenko');const expanded=r.completed===true&&r.mission===2&&r.campaignSize===undefined;return{mission:expanded?3:Number.isInteger(r.mission)?Math.max(0,Math.min(MISSIONS.length-1,r.mission)):0,unlocked,hero:unlocked.includes(r.hero)?r.hero:'shevchenko',completed:!expanded&&r.completed===true};}catch{return freshProgress();}
}
