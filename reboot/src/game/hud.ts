import {canExpansionUltimate} from './hero-expansion.ts';
import {TANK} from './mounts.ts';
import {canSpecial} from './hero-combat.ts';
...
export function mountAbilities(root:HTMLElement){
 ...
 return (world:World,keys:string[])=>{
  ...
  nodes.forEach((node,i)=>{
   ...
   const ammo=node.querySelector<HTMLElement>('.ammo-count')!;ammo.hidden=i!==0||(world.mounted?world.mounted.rounds===undefined:world.hero.magazine===0);if(i===0){ammo.textContent=String(world.mounted?.rounds??world.player.ammo);node.classList.toggle('reloading',world.player.reloading>0);}
