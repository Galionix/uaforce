import type {Mount} from './mounts.ts';import {TANK} from './mounts.ts';
/** Original tank on the same 16 px/world-unit grid, with a blue/yellow friendly marking. */
export function drawTankBody(c:CanvasRenderingContext2D,t:Pick<Mount,'dir'|'landing'|'armor'|'moving'|'recoil'|'hurt'>,x:number,y:number,time:number,occupied:boolean){
...
export function drawMount(c:CanvasRenderingContext2D,t:Mount,x:number,y:number,time:number,occupied:boolean){
 drawTankBody(c,t,x,y,time,occupied);
 const r=(a:number,b:number,w:number,h:number,color:string)=>{c.fillStyle=color;c.fillRect(Math.round(a),Math.round(b),w,h);};
 if(t.armor>0){
