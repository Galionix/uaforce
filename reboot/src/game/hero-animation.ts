...
export type HeroPose={grounded:boolean;ladder:number;wallClimbing:boolean;cast:number;attack:number;reloading:number;klychkoHold?:number};
export function heroFrame(p:HeroPose,clock:number,moving:boolean){
 ...
 if(moving)return 8+Math.floor(clock*16)%8;
 if((p.klychkoHold??0)>0)return 4;
