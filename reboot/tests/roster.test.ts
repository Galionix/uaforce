...
import {World,IDLE,type Enemy} from '../src/game/world.ts';import {HEROES,ACTIVE_HEROES,type HeroId} from '../src/game/content.ts';
...
test('eleven named heroes have complete, distinct kits, icons and sound profiles; no Sirko',()=>{
 assert.equal(HEROES.length,16);assert.ok(!HEROES.some(h=>(h.id as string)==='sirko'));
 for(const key of ['weapon','special','ultimate'] as const)assert.equal(new Set(HEROES.map(h=>h[key])).size,HEROES.length);
 assert.equal(new Set(HEROES.map(h=>HERO_ICONS[h.id].join(','))).size,HEROES.length);
 assert.equal(new Set(HEROES.map(h=>SFX_ASSETS[`${h.id}-special`].offset)).size,HEROES.length);
});
test('rescue unlocks all eleven without repeats and clears previous hero state',()=>{
 ...
 for(let i=1;i<=ACTIVE_HEROES.length;i++){const w=new World(0,unlocked,hero,()=>.999);w.mode='playing';w.enemies=[];w.player.x=w.allies[0].x;w.player.cloak=3;w.player.fireCount=2;w.step(1/60,{...IDLE,interact:true});assert.notEqual(w.heroId,hero);if(i<ACTIVE_HEROES.length){assert.ok(!unlocked.includes(w.heroId));assert.equal(w.unlocked.length,unlocked.length+1);}else{assert.equal(w.unlocked.length,ACTIVE_HEROES.length);assert.equal(w.cinematic,null);}assert.equal(w.player.cloak,0);assert.equal(w.player.fireCount,0);unlocked=w.unlocked;hero=w.heroId;}
 assert.equal(unlocked.length,ACTIVE_HEROES.length);
});
...
test('Mamai tap releases saber while deliberate hold fires a finite-range pistol',()=>{
 const near=fixture('mamai'),e=enemy(near,7);near.step(1/60,{...IDLE,fire:true});assert.equal(e.hp,1000);near.step(1/60,IDLE);assert.equal(e.hp,922);assert.equal(near.bullets.length,0);
 const far=fixture('mamai');enemy(far,12);for(let i=0;i<20;i++)far.step(1/60,{...IDLE,fire:true});assert.equal(far.shots,1);assert.ok(far.player.reloading>0);
});
...
test('Zelensky microphone fires on the third shot and allies provide friendly fire',()=>{const w=fixture('zelensky'),e=enemy(w,10);for(let i=0;i<3;i++){w.step(1/60,{...IDLE,fire:true});run(w,.4);}assert.equal(w.events.filter(e=>e.type==='voiceWave').length,1);assert.ok(e.panic!.remaining>0);w.step(1/60,{...IDLE,special:true});assert.equal(w.followers.length,2);run(w,1);assert.ok(e.hp<900);});
...
test('IT works without a radio and preserves charge at its owner-specific capacity',()=>{
 const w=fixture('it-army'),e=enemy(w,10);assert.equal(abilityStates(w)[1].ready,true);w.step(1/60,{...IDLE,special:true});assert.equal(w.specialCharges,1);assert.equal(w.followers.length,1);assert.equal(abilityStates(w)[1].ready,false);w.step(1/60,{...IDLE,special:true});assert.equal(w.specialCharges,1);run(w,2.6);assert.ok(e.hp<1000);
});
...
for(const h of HEROES)test(h.id+' practice selection, pause and respawn clear transient abilities',()=>{const w=practiceWorld(h.id);assert.equal(w.heroId,h.id);assert.equal(w.unlocked.length,HEROES.length);w.enemies=[];w.step(1/60,{...IDLE,special:true,ultimate:true});const before=JSON.stringify([w.effects.map(f=>[f.life,f.age]),w.player.specialRecovery]);w.mode='paused';run(w,1);assert.equal(JSON.stringify([w.effects.map(f=>[f.life,f.age]),w.player.specialRecovery]),before);w.mode='playing';w.player.invulnerable=0;w.damagePlayer(100);assert.equal(w.effects.length,0);assert.equal(w.followers.length,0);assert.equal(w.player.cloak,0);assert.equal(w.player.hp,100);});
