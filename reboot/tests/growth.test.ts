...
import worker,{cleanEvent} from '../server/pages-worker.js';
...
test('analytics stores only its whitelist; request headers and private extra properties are discarded',()=>{
 const value=cleanEvent({...point,room:'PRIVATE',url:'secret',email:'private@example.com'});
 assert.ok(value);assert.equal(JSON.stringify(value).includes('PRIVATE'),false);assert.equal(JSON.stringify(value).includes('email'),false);
 assert.equal(cleanEvent({...point,event:'PRIVATE'}),null);assert.equal(cleanEvent({...point,seconds:Infinity}),null);
});
