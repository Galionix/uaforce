...
test('host picks a remote player unlock once; guest receives the same fighter and reveal without drawing',()=>{
 ...
 const writer=new SnapshotWriter(host);host.stepPlayers(1/60,[IDLE,{...IDLE,interact:true}]);assert.equal(draws,1);assert.equal(host.players[0].heroId,'shevchenko');assert.equal(host.players[1].heroId,'skovoroda');
 const guest=new World(0,['shevchenko'],'shevchenko',()=>{throw Error('Guest must not draw');});const snapshot=JSON.parse(JSON.stringify(writer.snapshot(1,host.events)));applySnapshot(guest,snapshot);assert.equal(guest.heroId,'skovoroda');assert.deepEqual(guest.unlocked,host.unlocked);assert.deepEqual(guest.cinematic,host.cinematic);assert.equal(guest.events.find(e=>e.type==='heroChanged')?.hero,'skovoroda');
});
