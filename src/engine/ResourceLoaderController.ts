...
export class ResourceLoaderController {
  ...
  private async fetchFileFromServer(path: string) {
    const response = await fetch(
      `/api/resource/get?path=${encodeURIComponent(path)}`
    );
  ...
  public async fetchNewManifest(path: string): Promise<ResourceManifest> {
    const response = await fetch(
      `/api/resource/manifest?path=${encodeURIComponent(path)}`
    );
  ...
  //     return blob;
  //   }
  private isStoredManifestExpired(
  ...
  public async getResource(
    ...
    if (
      !oldManifest ||
      this.isStoredManifestExpired(oldManifest?.lastFetchedAt)
    ) {
